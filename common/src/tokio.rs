...
pub trait AsyncSizedMessage {
    fn write_sized<'a>(&'a mut self, buf: &'a [u8]) -> impl Future<Output = tokio::io::Result<()>> + Send + 'a;
    fn read_sized<'a>(&'a mut self) -> impl Future<Output = tokio::io::Result<Vec<u8>>> + Send + 'a;
}
    ...
    fn write_sized<'a>(&'a mut self, buf: &'a [u8]) -> impl Future<Output = tokio::io::Result<()>> + Send + 'a {
        async move {
            ...
            let buf_len: u32 = buf.len()
                .try_into()
                .map_err(|_| tokio::io::Error::new(std::io::ErrorKind::InvalidData, "message too large"))?;
